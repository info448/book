...
An important point to note: does this Activity have a **constructor** that we call? _No!_ We never write code that **instantiates** our Activity (that is: we never call `MainActivity()`). There is no `main()` method in Android. Activities are created and managed by the Android operating system when the app is launched.
...
- In the Java code, we want to add more functionality to the `ActionBar`. *Think*: which lifecycle callback should this specification be put in?
	...
	```kotlin
	//kotlin
	//can access the supportActoinBar directly (though assert it is not null)
